...
def save_document(message):
    if message.document:
        ...
        deal_id = clients.get(message.chat.id)  # Получаем ID сделки из CRM
        if not deal_id:
        ...
        payload = {
            "id": deal_id,
            "fields": {
                "UF_CRM_XX_DOCUMENTS": [{"fileData": ["document.pdf", file_url]}]  # Исправлено
            }
        }
        response = requests.post(f"{BITRIX_WEBHOOK}crm.deal.update.json", json=payload)
        data = response.json()
...
def view_case_status(message):
    phone_number = clients.get(message.chat.id, None)
    if not phone_number:
        bot.send_message(message.chat.id, "❌ Ошибка: Не найден номер телефона.")
        return
    check_status_in_crm(message, phone_number)
