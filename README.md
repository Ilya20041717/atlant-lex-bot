...
### Приветствие до нажатия «Старт»

Текст в профиле бота (виден новому пользователю до нажатия «Запустить») задаётся в `app/content/premium.py`: `BOT_SHORT_DESCRIPTION` (до 120 символов) и `BOT_DESCRIPTION`. Чтобы применить их к боту, один раз выполните из корня проекта: `python scripts/set_bot_welcome.py` (нужен `BOT_TOKEN` в `.env`).
